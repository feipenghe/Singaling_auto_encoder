...
As a sampling procedure for artificial messages, we take the average of 10 messages from a (now labelled) cluster. These artificial messages are fed to Receiver.
We repeat this process \nbSST{Nur: XXX times}. The output object accuracy for these artificial messages is given in Table~\ref{tab:average_message_accuracy}. The model achieves recovery accuracy similar to when interpreting actual messages. %This can be paralleled with the phenomenon of Categorical Perception, which describes how continuous signals, such as phonemes in an acoustic space, are perceived as stable and discrete, even when the signal is gradually shifted in the continuous space.
...
We find that passing the right-hand side of the equation above (that is the compositionally appropriate message, in principle) to the Receiver in place of the left-hand side leads to important degradation of communicative success (a drop of at least 36 percentage points across parameter combinations, to around chance level).
\nbSST{Nur: is there a way of being as succinct but slightly more precise with the experimental set up here?  E.g. ``For XXX contexts, and every $f$, we compared passing \MESS{$\argmax_i$} to \dots to the Receiver, and found\dots.

I'm not 100\% sure of the setup, so the reader might not as well.  Should it be \MESS{$c, \argmax_i$ as well, or am I missing something?}}
