...
\nbEC{Note here on in the discussion that we reach communicative success with a large number of objects, compared to previous work}
...
\end{tabular}
\caption{Communicative success (object recovery accuracy) for the different model settings.}\label{tab:object_prediction_accuracy}
\end{table}
...
\end{tabular}
\caption{Discreteness in production (F1 scores for produced messages) for the different model settings.}
...
\end{tabular}
\caption{Discreteness in perception (object recovery accuracy from artificial messages) for the different model settings.}
%\caption{Message clusterization F1 scores.}
...
\MESS{$\argmax_i$}=\MESS{$\argmax_j$}-\MESS{$\argmin_j$}+\MESS{$\argmin_i$}. 
\nbEC{describe results here}
...
\MESS{$\argmax_i$} from \MESS{$\argmax_j$}, \MESS{$\argmin_j$} and \MESS{$\argmin_i$} (therefore leaving out the assumption that this relation would be addition). Leaving out one dimension $i_0$ from training, we ask whether the network was able to recover the left-out \MESS{$\argmax_{i_0}$} messages from their corresponding \MESS{$\argmax_j$}, \MESS{$\argmin_j$} and \MESS{$\argmin_{i_0}$}.
\nbEC{describe results here}
